...
#let note(it) = {
  set enum(numbering: "(1)")
  let body = [*注记.*] + h(config.block-space) + it
  block(
    inset: 8pt,
    below: config.leading,
    width: 100%,
...
// 引入框
#let intro = custom-block.with(
  title: [引入.],
  color: config.intro-color,
)

// 证明框
#let proof = custom-block.with(
  title: [证明.],
  color: config.proof-color,
)
...
#let make-title(
  ...
  // set text(config.title2-size, weight: "bold", font: config.title-font)
  // align(center, "摘要")
  // set text(config.author-size, weight: "light", font: config.author-font)
...
#let project(
  ...
  set page(
    ...
    header: context {
      ...
      grid(
        columns: (20fr, 0fr),
        align(center, title), /*align(right, heading),*/
        // align(center, "A 题：传感器阵列流量调度问题")
      )
      v(-1.2em)
  ...
  show heading: it => {
    ...
    let body = if it.numbering != none and counter(heading).display() != "六" and counter(heading).display() != "七" {
      // it.body
